...
export default defineWrappedResponseHandler(async (event) => {
  const slug = z.string().min(1).parse(getRouterParam(event, 'slug'))
  ...
  try {
    ...
    const categoryJoin = await db
      ...
      .where(eq(schema.contentCategories.contentId, post.id))
      .limit(1)

    return {
      ...post,
      category: categoryJoin[0] ?? null,
