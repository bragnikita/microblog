...
export async function getDsqlDb(): Promise<DbConnection> {
  ...
  const client = new AuroraDSQLClient({
    ...
    user: 'admin',
    database: 'postgres',
    profile: process.env.CI ? undefined : (process.env.AWS_PROFILE || undefined), // Use AWS profile in local dev, but not in CI (where credentials are provided via env vars)
